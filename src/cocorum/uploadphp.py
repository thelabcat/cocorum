...
class UploadResponse(JSONObj):
    ...
    @property
    def fid_b10(self) -> int:
        """The numeric ID of the uploaded video in base 10"""
        return self.fid

    @property
    def fid_b36(self) -> str:
...
class UploadPHP:
    ...
    def __init__(self, servicephp: ServicePHP):
        ...
        """A ServicePHP instance to authenticate with"""

        self.scraper: scraping.Scraper = scraping.Scraper(self.servicephp)
        """A scraper to get miscellaneous data we need"""

        self.channels: list[scraping.HTMLChannel] = self.scraper.get_channels()
        """List of channels we can optionally use for upload"""
    ...
    def uphp_request(self, additional_params: dict, method: str = "PUT", data: Optional[dict] = None, timeout: int | float = static.Delays.request_timeout) -> requests.Response:
    ...
    def ensure_valid_channel_id(self, channel_id) -> int | None:
    ...
    def _chunked_vidfile_upload(self, file_path: str) -> str:
        ...
        with open(file_path, "rb") as f:
            for i in range(self.__cur_num_chunks):
                ...
                # Upload the chunk
                self.uphp_request(chunk_params, data=f.read(
                    static.Upload.chunksz), timeout=static.Delays.upload_request_timeout)
    ...
    def _unchunked_vidfile_upload(self, file_path: str) -> str:
    ...
    Returns:
        ...
        with open(file_path, "rb") as f:
            ...
            # Upload the file
            r = self.uphp_request(
                {}, data=f.read(), timeout=static.Delays.upload_request_timeout, method="POST")

        uploaded_fn = r.text

        # TODO issue #24
        assert len(uploaded_fn) < 100, "Uploaded filename too long to be correct"

        print("Video file on server is", uploaded_fn)
        return uploaded_fn

    def _upload_cthumb(self, file_path: str) -> str:
    ...
    def upload_video(self, file_path: str, title: str, category1, **kwargs) -> UploadResponse:
        ...
        assert isinstance(
            category1, (int, float)), f"Primary category must be number or str name, got {type(category1)}"
        ...
        assert isinstance(
            category2, (int, float)) or category2 is None, f"Secondary category must be number or str name, got {type(category1)}"
        ...
        updata = {
            ...
            "file_meta": {
                ...
                # Timestamp file was modified, miliseconds
